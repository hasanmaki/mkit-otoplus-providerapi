...
async def request_handler(client: httpx.AsyncClient, methode: str, url: str, **kwargs):
    ...
    try:
        ...
        log.error(f"Connection Error to {url}: {exc.__class__.__name__}")
        ...
        log.exception(f"Response Status Error: {exc.response.status_code}")
